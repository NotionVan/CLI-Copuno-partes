...
Definidos en [.Codex/agents/](.Codex/agents/). Invocar con `@<nombre>` cuando aplique:

| Agente | Cuándo invocarlo | Tools |
|---|---|---|
| [`@senior-architect-auditor`](.Codex/agents/senior-architect-auditor.md) | Antes de refactors mayores, al planificar nuevos módulos o cuando se necesita un análisis arquitectónico estructurado con severidad + lente ROI. | Read, Grep, Glob, Bash (opus) |
| [`@notion-integration-inspector`](.Codex/agents/notion-integration-inspector.md) | Antes de tocar la capa Notion: esquema, queries, sync, `invalid_grant`, "app no actualiza". | Read, Grep, Glob, Bash |
| [`@regression-checker`](.Codex/agents/regression-checker.md) | **Antes de mergear cualquier cambio.** Verifica firma, PDF y sync Notion. | Read, Grep, Glob, Bash |
| [`@scope-guardian`](.Codex/agents/scope-guardian.md) | Cada petición nueva del cliente Copuno: ¿retainer o proyecto aparte? | Read |
...
- Cualquier ampliación grande del sistema (módulos nuevos, integraciones Chorus/OneNote/WhatsApp, portal del empleado) es **proyecto aparte**. Ver [.Codex/scope-rules.md](.Codex/scope-rules.md).
