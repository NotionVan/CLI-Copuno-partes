...
- **pdf.co está en el camino crítico de la firma, con DOS operaciones por parte** (PARTES4/4: módulo 31 `AnythingToPDF` convierte el documento de OneDrive a PDF, módulo 32 `pdfEditAdd` estampa la firma en x=360/y=660). **Sustituirlo se evaluó y se DESCARTÓ el 18-08**: el plan Basic son 8,99 $/mes con 16.500 créditos frente a los 400-600 proyectados para octubre (27-40× de margen), así que las 6-10 h de desarrollo equivalen a más de 5 años de suscripción. Es técnicamente viable —OneDrive convierte a PDF de forma nativa con `?format=pdf`, y `pdf-lib` estampa la imagen sin binarios— pero no compensa. **Lo que sí urge: salir del plan gratuito**, porque su agotamiento significa «no se puede firmar ningún parte» y septiembre arranca con 3 usuarios firmando a diario. Detalle completo en [docs/DEUDA_TECNICA.md](docs/DEUDA_TECNICA.md).
- **La reversión de despliegue NO es instantánea ni atómica** (ensayada en producción el 18-08, v1.13.4→v1.13.3→v1.13.4, muestreo de `/api/health` cada segundo, n=150): **~12 s** hasta que producción sirve la versión anterior, **2 s de convivencia** en los que distintas peticiones reciben respuestas de despliegues distintos, y **un fallo diferido a los ~42 s** (arranque en frío de la instancia nueva, 1 de 150 peticiones). **Medio minuto largo hasta estabilidad completa.** Antes de revertir, **verificar en el panel cuál es el despliegue ACTIVO**: los commits de `docs:` también generan despliegue, así que el activo no tiene por qué ser el que uno recuerda (en el ensayo, el activo era el commit de documentación, no el de código). El banner de versión de la app sí avisa al usuario con sesión abierta — confirmado en el ensayo. Procedimiento completo en [docs/RESULTADOS_RENDIMIENTO_2026-08.md](docs/RESULTADOS_RENDIMIENTO_2026-08.md) §12.4b.
- **La cuenta personal de Vercel no ve el proyecto del cliente.** El CLI y el conector MCP autentican contra `javiers-projects`; el proyecto vive en `copunos-projects`. Cualquier operación sobre producción (revertir, promover, leer Speed Insights) requiere la sesión del navegador con el equipo del cliente seleccionado — no se puede automatizar desde aquí.
