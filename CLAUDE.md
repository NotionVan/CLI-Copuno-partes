...
Los blueprints JSON de los escenarios Make.com se guardan en [docs/Escenarios Make/](docs/Escenarios%20Make/), pero **NO están versionados en git y no deben estarlo**: la carpeta está en `.gitignore` porque los blueprints contienen **secretos en texto plano** — un token de integración de Notion (`Bearer ntn_…`) hardcodeado en un módulo HTTP, y las URLs de los webhooks. Commitearlos filtraría el acceso al workspace del cliente al historial de git de forma permanente.

Consecuencia: **la referencia canónica del lado Make es producción (eu2), no el repo.** Los ficheros locales son una foto que se queda obsoleta en cuanto alguien toca un escenario en la UI, y no hay diff que avise. Antes de auditar o diagnosticar, **redescargar siempre desde la API**:

```bash
set -a && . ./.env && set +a   # MAKE_TOKEN (token de API Make, solo lectura)
...
Si algún día se quiere versionar, hay que **sanear antes** (sustituir el token por un placeholder) y, mejor aún, migrar ese módulo HTTP a la conexión nativa de Notion para que el secreto deje de viajar en el export.
...
- **Si cambias el payload de un tramo, redetermina la estructura del webhook receptor.** Un campo nuevo que el webhook no conoce aparece como *variable desconocida* y **resuelve vacío en silencio**: sin error, sin ejecución incompleta, sin log — sólo se detecta mirando el PDF final. Pasó con `Vehiculos del parte` entre PARTES1/4 y PARTES2/4 (DEUDA_TECNICA M8): las matrículas se perdían en el tramo 2/4 y el PDF salía sin ellas, pese a que la prueba E2E de I6 había pasado semanas antes. Regla: tras tocar el payload, *Redetermine data structure* en el receptor + **validación E2E mirando el PDF**, no sólo que el escenario termine en verde.
